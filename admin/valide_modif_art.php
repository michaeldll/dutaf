...
$requete = 'SELECT *  FROM articles WHERE art_num='.$_GET['numInput'];
...
<div class="row">
    <div class="col-sm-offset-4 col-sm-4 text-center">
        <h1>Modifications effectuées sur <?php echo $ligne['art_desi']; ?></h1>
        <?php $requete_2 = 'UPDATE articles SET  art_num='.$_GET["numInput"].',
                                                 art_desc="'.$_GET["descInput"].'",
                                                 art_desi="'.$_GET["desiInput"].'",
                                                 art_prix='.$_GET["prixInput"].',
                                                 art_qua='.$_GET["quaInput"].' 
                                                 WHERE art_num='.$_GET["numInput"];
        // UPDATE articles SET art_num=2,art_desc="desci",art_desi="desig",art_prix=3,art_qua=4 WHERE art_num=2 
        $exe = $bdd->query($requete_2); ?>
        <a href="gest_art.php">Retourner</a>
    </div>
</div>
</div>
